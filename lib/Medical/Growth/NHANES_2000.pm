...
sub measure_class_name_for {
  ...
  croak('Need to specify measure, age_group, and sex')
    unless exists $criteria{measure} and
           exists $criteria{age_group} and
	   exists $criteria{sex};

  if ($criteria{measure} =~ /^(\w+)(?:\s+|_)(?:for|by)(?:\s+|_)(\w+)/i or
      $criteria{measure} =~
  ...
  elsif ($criteria{age_group} =~ /^[0-9.]+$/ and 
	 ($criteria{age_group} eq '0' or $criteria{age_group} > 0)) { 
  ...
  given ($measure) {
    ...
    default               { croak "Don't understand measure name '$measure' in " .
			          "'$criteria{measure}'"; }
  }
  ...
  given ($norm) {
    ...
    default             { croak "Don't understand norm name in " .
			        "'$criteria{measure}'"; }
  }
  ...
  $class = 'Medical::Growth::NHANES_2000::' .
           $measure . '_for_' . $norm . '::' .
	   $ages . '::' . $sex;
  return $class;
}
