...
is_deeply($h->_params_LMS,
	 [ { index => 1, L => 0, M => 1, S => 1 },
	   { index => 2, L => 2, M => 2, S => 2 },
	   { index => 3, L => 3, M => 3, S => 3 } ],
	 '_build_params_LMS');

is_deeply([ $h->lookup_LMS(2) ],
	  [ 2, 2, 2 ],
	  'lookup_LMS: exact match');

is_deeply([ $h->lookup_LMS(1.5) ],
	  [ 1, 1.5, 1.5 ],
...
is( int($h->z_for_value(2,1) * 1000) / 1000 ,0.693,'z_for_value: index in range, l == 0');
...
is( int($h->value_for_z(0.69,1) + 0.01) ,2,'z_for_value: index in range, l == 0');
